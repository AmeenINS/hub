...
import type { Lead } from '@/shared/types/database';
...
export default function EditLeadPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  }, [resolvedParams.id, canWrite, isLoading]);
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setFormData({
          ...
          currentPremium: lead.currentPremium?.toString() || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiClient.put(`/api/crm/leads/${resolvedParams.id}`, payload);
      if (response.success) {
        toast.success(t('crm.leadUpdated'));
        router.push(`/dashboard/crm/leads/${resolvedParams.id}`);
      }
    } catch (error) {
      toast.error(getErrorMessage(error, t('crm.failedToUpdate')));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex justify-end gap-4 mt-6">
          <Button type="button" variant="outline" onClick={() => router.push(`/dashboard/crm/leads/${resolvedParams.id}`)} disabled={loading}>
            {t('common.cancel')}
          </Button>
